...
import { cors, json, requireAdmin, server, getIssuer, getAsset, validAmount, findTrustline, availableBalance, NETWORK_PASSPHRASE, errorMessage } from './_config.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [source, holder] = await Promise.all([server.loadAccount(issuer.publicKey()), server.loadAccount(from)]);
